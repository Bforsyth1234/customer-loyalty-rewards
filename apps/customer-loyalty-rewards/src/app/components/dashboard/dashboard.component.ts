  ...
  template: `
    <div class="dashboard bg-gray-100 min-h-screen p-8">
      <div class="max-w-6xl mx-auto">
        ...
        <div *ngIf="userRewards" class="mb-8 p-4 bg-white rounded-lg shadow-md">
          <h2 class="text-xl font-semibold mb-4">User Rewards Information</h2>
          <p class="text-lg">Name: <span class="font-bold">{{ userRewards.first_name }} {{ userRewards.last_name }}</span></p>
          <p class="text-lg">Phone: <span class="font-bold">{{ userRewards.phone }}</span></p>
          <p class="text-lg">Total Points: <span class="font-bold">{{ userRewards.total_points }}</span></p>
            ...
            <div class="flex flex-wrap gap-4">
              <button 
                *ngFor="let reward of availableRewards"
                ...
                [disabled]="!userRewards || userRewards.total_points < reward.point_cost"
                class="btn-gradient py-2 px-4 rounded-md text-sm flex-grow flex flex-col items-center justify-center min-w-[150px] h-24"
                [ngClass]="{'opacity-50 cursor-not-allowed': !userRewards || userRewards.total_points < reward.point_cost}"
              >
...
export class DashboardComponent implements OnInit {
  ...
  userRewards: UserRewards | null = null;
  ...
  constructor() {
    ...
    if (navigation?.extras.state) {
      this.userRewards = navigation.extras.state['userData'] as UserRewards;
    }
  }
  ...
  async fetchUserRewards() {
    ...
    if (user) {
      console.log('Fetching rewards for user:', user.email);
      ...
      if (error) {
        ...
        console.log('Fetched user rewards:', data[0]);
        this.userRewards = data[0];
      } else {
  ...
  private async addPoints(points: number, activity: string) {
    if (this.userRewards) {
      const updatedPoints = this.userRewards.total_points + points;
      console.log(`Attempting to add ${points} points for ${activity} to user:`, this.userRewards);
      ...
      const { data, error } = await this.supabase
        ...
        .eq('phone', this.userRewards.phone)
        .select();
      ...
      if (error) {
        ...
        console.error('No user found with the given phone number:', this.userRewards.phone);
      }
    } else {
      console.error('No user rewards data available');
    }
  }
  ...
  async redeemReward(reward: AvailableReward) {
    if (this.userRewards && this.userRewards.total_points >= reward.point_cost) {
      const updatedPoints = this.userRewards.total_points - reward.point_cost;
      console.log('Attempting to redeem reward for user:', this.userRewards);
