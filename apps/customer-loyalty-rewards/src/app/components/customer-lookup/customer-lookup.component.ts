...
@Component({
    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-2xl">
        ...
        <button (click)="openAddUserModal()"
          ...
          <fa-icon [icon]="faUserPlus" class="mr-2"></fa-icon> Add User
        </button>
    ...
    <!-- Add User Modal -->
    <div *ngIf="isAddUserModalOpen" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
...
export class CustomerLookupComponent {
    ...
    async lookupRewards(phone: string) {
        ...
        if (error) {
            ...
            // Navigate to the dashboard with the new user's data
    ...
    onAddUserSubmit(): void {
        if (this.addUserForm.valid) {
            const phone = this.addUserForm.value.phone?.replace(/\D/g, '') ?? '';
            const totalPointsString = this.addUserForm.value.totalPoints ?? '0'; // Ensure totalPoints is a string
            const total_points = parseInt(totalPointsString, 10); // Convert totalPoints to an integer
            const first_name = this.addUserForm.value.firstName ?? '';
            const last_name = this.addUserForm.value.lastName ?? '';
            this.addUser({ phone, total_points, first_name, last_name });
        }
    }

    async addUser(rewards: Rewards) {
        const user = await this.authService.user$.pipe(take(1)).toPromise();
        if (user) {
            const rewardsWithEmail = { ...rewards, email: user.email };
            const { data, error } = await this.supabase
                .from('customer_rewards')
                .insert([rewardsWithEmail])
                .select()
                .single();
